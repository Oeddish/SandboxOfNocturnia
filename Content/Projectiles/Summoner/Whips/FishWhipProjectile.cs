...
{
    internal class FishWhipProjectile : ModProjectile
    {
        ...
        {
            target.GetGlobalNPC<FishWhipTagGlobalNPC>().taggedByWhip = true;
            ...
            for (int i = 0; i < numDust; ++i)
            {
                ...
                int idx = Dust.NewDust(target.position, target.width, target.height, DustID.Web);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                Rectangle frame = new Rectangle(0, 0, 6, 36); // The size of the Handle (measured in pixels)
                Vector2 origin = new Vector2(0, 0); // Offset for where the player's hand will start measured from the top left of the image.
                float scale = 1;
                ...
                if (i == list.Count - 2)
                {
                    ...
                    frame.Y = 44; // Distance from the top of the sprite to the start of the frame.
                    frame.Height = 8; // Height of the frame.
                    frame.Width = 8;
                    origin -= new Vector2(-4f, 50f);
    ...
    public class FishWhipTagGlobalNPC : GlobalNPC
    {
        public bool taggedByWhip = false;
        public int countTag = 0;

        public override void ResetEffects(NPC npc)
        {
            if (taggedByWhip == true) { countTag = 300; }
            taggedByWhip = false; // Reset each frame
