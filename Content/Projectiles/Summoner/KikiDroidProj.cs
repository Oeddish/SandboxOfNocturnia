...
{
	public class KikiDroidProj : ModProjectile
	{
		int atkSpd;
		...
		public override void AI()
		{
			...
			if(!checkPartner())
			{
				int bunID = Projectile.NewProjectile(
					Projectile.GetSource_FromThis(),
					Projectile.Center,
					Vector2.Zero,
					ModContent.ProjectileType<KikiBunProj>(),
					30,
					Projectile.knockBack*1.5f,
					owner.whoAmI,
					ai0: Projectile.ai[0] // Pass owning projectile ID
					);
		...
		private bool checkPartner()
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				...
				if (proj.active && proj.owner == player.whoAmI && proj.minion && proj.type == ModContent.ProjectileType<KikiBunProj>() && proj.ai[0] == Projectile.ai[0])
				{
					return true;
				}
			}
		...
		private bool CheckActive(Player owner)
		{
			...
			{
				owner.ClearBuff(ModContent.BuffType<KikiSummonBuff>());

				return false;
			}

			if (owner.HasBuff(ModContent.BuffType<KikiSummonBuff>()))
			{
			...
			if (foundTarget)
			{
				...
				if (atkSpd == 0)
				{
					...
					Projectile.NewProjectile(
						...
						Projectile.Center,
						direction * 3f,
						ModContent.ProjectileType<DroneBomb>(),
						1,
						Projectile.knockBack,
						Projectile.owner,
						npcID //Give the new projectile the ID of the NPC it is targetting
						);
				}
				atkSpd++;
				if (atkSpd >= 180)
