...
namespace FragmentsOfNocturnia.Content.Projectiles.Tools
{
    internal class NocturneDrillProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Vector2 playerCenter = player.RotatedRelativePoint(player.MountedCenter) + new Vector2(0f, 5f);
            if (Main.myPlayer == Projectile.owner)
            {
                // This code must only be ran on the client of the projectile owner
                if (player.channel)
                {
                    float holdoutDistance = player.HeldItem.shootSpeed * Projectile.scale - 15f;
            ...
            {
                Dust dust = Dust.NewDustDirect(Projectile.position + Projectile.velocity * Main.rand.Next(6, 10) * 0.15f, Projectile.width, Projectile.height, ModContent.DustType<NocturneDust>(), 0f, 0f, 80, Color.Pink, 1f);
                dust.position.X -= 4f;
                dust.noGravity = true;
